...
class P extends CI_Controller {
	...
	public function reg_action()
	{
		...
		if ($password != $repassword) {
			...
			$result = $this->Kas_model->simpan('login', $data);
			if ($result > 0) {
	...
	function ganti_password_act(){
		...
		if($this->form_validation->run() != false){
			...
			$this->Kas_model->update_data($w,$data,'login');			
	...
	function laporan_harian_print(){					
		$tanggal = $this->input->get('tanggal');		
		
		if($tanggal){					
			$data['result'] = $this->db->query("select * from data where date(tanggal) = '$tanggal'")->result();											
			$this->load->view('laporan_harian_print',$data);			
		}else{
			redirect("p/laporan_harian");	
		}		
	}

	function laporan_harian_pdf(){
		$this->load->library('dompdf_gen');	
		$tanggal = $this->input->get('tanggal');		
			
		$data['result'] = $this->db->query("select * from data where date(tanggal) = '$tanggal'")->result();											
		$this->load->view('laporan_harian_pdf', $data);
	...
	function laporan_periode_print(){					
		$tgl_mulai = $this->input->get('tgl_mulai');
		$tgl_sampai = $this->input->get('tgl_sampai');		
		
		if($tgl_mulai != "" && $tgl_sampai != ""){					
			$data['result'] = $this->db->query("select * from data where date(tanggal) >= '$tgl_mulai'")->result();											
			$this->load->view('laporan_periode_print',$data);			
		}else{
			redirect("p/laporan_print");	
		}		
	}

	function laporan_periode_pdf(){
		$this->load->library('dompdf_gen');	
		$tgl_mulai = $this->input->get('tgl_mulai');
		$tgl_sampai = $this->input->get('tgl_sampai');		
			
		$data['result'] = $this->db->query("select * from data where date(tanggal) >= '$tgl_mulai'")->result();											
		$this->load->view('laporan_periode_pdf', $data);
